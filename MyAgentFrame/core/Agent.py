...
class Agent(ABC , BaseModel):
    ...
    llm: BaseChatLLM = Field(default_factory=BaseChatLLM , alias="llm")
    """LLM基类"""

    system_prompt: Optional[str] = Field(default=None, alias="system_prompt")
    """SP"""

    config: Config = Field(default_factory=Config, alias="config")
    """参数配置"""

    tool_registry: Optional['ToolRegistry'] = Field(default=None, alias="tool_registry")
    """工具注册表"""

    history_manager : HistoryManager = Field(default_factory=HistoryManager, alias="history_manager")
    """会话历史管理与压缩"""

    @model_validator(mode='after')
    def _init_history_manager(self):
        """根据 config 初始化 history_manager"""
    ...
    trace_logger: Optional['TraceLogger'] = Field(default=None, alias="trace_logger")
    """追踪日志"""

    @model_validator(mode='after')
    def _init_trace_logger(self):
        """根据 config 初始化 session_store"""
    ...
    skill_loader: Optional[SkillLoader] = Field(default=None, alias="skill_loader")
    """技能加载器"""
    ...
    session_store: Optional[SessionStore] = Field(default_factory=SessionStore)
    """会话持久化"""
    ...
    # 会话元数据（用于保存）
    _session_metadata: Dict[str, Any] = PrivateAttr(default_factory=lambda: {
    ...
    #  新增token计数 - 使用私有属性，不参与初始化
    _history_token_count: int = PrivateAttr(default=0)
    ...
    def token_counter(self, value: TokenCounter):
        self._token_counter = value

    @model_validator(mode='after')
    def _init_task_tool(self):
        """根据 config 初始化 子代理组件"""
    ...
    # 新增：TodoWrite 进度管理组件
    @model_validator(mode='after')
    def _init_todowrite_tool(self):
        """根据 config 初始化 进度管理组件"""
    ...
    # 新增：DevLog 开发日志组件
    @model_validator(mode='after')
    def _init_devlog_tool(self):
        """根据 config 初始化 开发日志组件"""
    ...
    @_history.setter
    def _history(self, value: List[BaseMessage]):
        """向后兼容：允许直接设置历史"""
    ...
    def _register_task_tool(self):
        ...
        # 创建 Agent 工厂函数
        from ..tool.builtin import TaskTool
        ...
        def agent_factory(agent_type: str) -> Agent:
            ...
            # 使用默认工厂创建子代理
            from ..agent import default_subagent_factory
    ...
    def _generate_simple_summary(self, history: List[BaseMessage]) -> str:
        """生成简单摘要（统计信息）
    ...
    def _generate_smart_summary(self, history: List[BaseMessage]) -> str:
        """生成智能摘要（调用 LLM）
    ...
    def _format_history_for_summary(self, history: List[BaseMessage]) -> str:
        """格式化历史消息用于摘要生成
    ...
    def _execute_tool_call(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        if tool:
           try:
                # 处理参数类型
                type_arguments = self._convert_parameter_types(tool_name, arguments)
                # 执行工具
                response = tool.run_with_timing(type_arguments)
    ...
    def load_session(self, filepath: str, check_consistency: bool = True) -> None:
        ...
        for msg_data in session_data.get("history", []):
            self.history_manager.append(BaseMessage.from_dict(msg_data))
    ...
    def _compute_tool_schema_hash(self):
        """计算工具 Schema 哈希
        ...
        for tool_name in sorted(self.tool_registry.list_tools()):
            ...
            if tool:
                tools_signature[tool_name] = {
                    ...
                    "parameters": list(tool.parameters.keys()) if hasattr(tool,
                                                                          'parameters') and tool.parameters else []
                }

        schema_str = json.dumps(tools_signature, sort_keys=True)
        return sha256(schema_str.encode()).hexdigest()[:16]

    def _get_read_cache(self):
        """获取 Read 工具的元数据缓存
  ...
  # ======================= 子代理执行 =========================
    ...
    def _extract_tools_from_history(self, history: List[BaseMessage]) -> List[str]:
        """从历史中提取使用的工具
    ...
    def _register_todowrite_tool(self):
        ...
        # 创建并注册 TodoWriteTool
        from ..tool.builtin import TodoWriteTool
    ...
    def _register_devlog_tool(self):
        ...
        from ..tool.builtin import DevLogTool
    ...
    def _create_light_llm(self) -> BaseModel:
        """创建轻量模型 LLM 实例

...
class Planner(BaseModel):
    ...
    llm_client: BaseChatLLM = Field(default_factory=BaseChatLLM , alias="llm")
    """ llm """

    system_prompt: str = Field(default = """你是一个顶级的AI规划专家。你的任务是将用户提出的复杂问题分解成一个由多个简单步骤组成的行动计划。
请确保计划中的每个步骤都是一个独立的、可执行的子任务，并且严格按照逻辑顺序排列。""", alias="prompt")
...
class Executor(BaseModel):
    ...
    system_prompt : Optional[str] = Field(default="""你是一位顶级的AI执行专家。你的任务是严格按照给定的计划，一步步地解决问题。
请专注于解决当前步骤，并输出该步骤的最终答案。""" , alias="prompt")
    """ system_prompt 系统提示词 """

    tool_registry : Optional[ToolRegistry] = Field(default =  None)
    """ tool_registry 工具注册器 """

    enable_tool_calling : bool = Field(default = True)
    """ enable_tool_calling """
...
class PlanSolveAgent(Agent , BaseModel):
    """
    ...
    # 使用 PrivateAttr 声明私有属性（不会作为 Pydantic 字段验证）
    _planner: Optional[Planner] = None
    _executor: Optional[Executor] = None

    @model_validator(mode='after')
    def _initialize_components(self):
        """在模型验证后初始化 Planner 和 Executor"""
        # 确保 llm 已经存在（从父类 Agent 继承）
        if not hasattr(self, 'llm') or self.llm is None:
            raise ValueError("PlanSolveAgent 需要配置 llm 参数")
    ...
    def planner(self) -> Planner:
        ...
        if self._planner is None:
            raise RuntimeError("Planner 未初始化，请先完成模型验证")
    ...
    def executor(self) -> Executor:
        ...
        if self._executor is None:
            raise RuntimeError("Executor 未初始化，请先完成模型验证")
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        if  not plan:
            ...
            self.add_message(BaseMessage(final_answer, "assistant"))
            self.add_message(BaseMessage(input_text, "user"))
        ...
        **kwargs
        ...
        try:
            ...
            if not plan:
                ...
                self.add_message(BaseMessage(input_text, "user"))
                self.add_message(BaseMessage(error_msg, "assistant"))
                return
